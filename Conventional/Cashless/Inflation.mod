/*
 * A ___standard___ New Keynesian Model for __Cashless Economies__
 * Considering a ____inflation shock______ 
 */


var  y,c,ce,m,k,i, s, pi,x,b,a, tec_shock, pref_shock, inf_shock, beta_t,r; 
 ...
 */
chi   = 0.00; // Degree of non-separability of utility function
...
rp = 1.005; //Constant risk premium
...
[mcp = 'r > -0.0203']
r = phi_s*s(-1)+(1-phi_s)*(phi_pi*pi+phi_y*y);

[name = 'IS Curve']
c = c(+1)-sigma*(r+beta_t(+1)-pi(+1)+chi*(m(+1)-m));
[name = 'LM Curve']
m = eta_c*c-eta_r*r;

[name = 'Entrepreneur restriction']
CE*ce = (alpha*(y-x)/X)+B*b-I*i-(r(-1)+b(-1)-pi)*B/beta;
[name = 'Lending constraint']
b = pi(+1)+k-r;
[name = 'Entrepreneur Euler']
0 = (1-LV*beta)*(ce-ce(+1))+(LV*beta)*(pi(+1)-r)+(alpha*gamma*delta/(X*I))*(y(+1)-x(+1)-k);
...
perfect_foresight_solver(tolf=1e-10);
perfect_foresight_solver(tolx = 1e-10);
